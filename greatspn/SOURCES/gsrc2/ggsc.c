...
void tofileSMART(char *netname)
{
   ...
   fprintf(ctmcfile,"for (int i in {0..%d})\n\t{\n\tstate s[i];\n\t}\n", limit_tan-1);
   ...
   for ( m_c_p=column+1 ; m_c_p<last_column ; m_c_p++ )
	  {
	   ...
	   for (i=m_c_p->ninn, item=m_c_p->first ; i>0 ; i--, item++)
	     ...
	     if (first==0)
	      {
	      first=1;
	      fprintf(ctmcfile,"s[%d]:s[%d]:%f", item->fm-1,indice-1, dval[item->rt]/dval[(column+item->fm)->weight]);
	      }
	     else
	     fprintf(ctmcfile,", s[%d]:s[%d]:%f", item->fm-1,indice-1, dval[item->rt]/dval[(column+item->fm)->weight]);
             count_arcs++;
