...
void getarcs (ARCM t_arc,ARCM p_arc, int noar, int notr);


struct place_obj { ROWT mark;
		   ROWT enc;
		   ROWT mult;
		   ROWT covered;
		   ROWT ubound;
...
struct T_list { ROWT trans;
	        struct T_list * next;
	      };

typedef struct T_list * T_L_P;

struct I_header { ROWT nouncov;
		  ROWT cod_type;
		  ROWT tcount;
		  ROWP rowp;
