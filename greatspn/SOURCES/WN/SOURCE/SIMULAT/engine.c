...
/* RETURN VALUE : */
/**************************************************************/
static void close_intermediate_files(file)
 char file[MAXSTRING];
 {/* Init close_intermediate_files */
   ...
   fclose(timefp);
   fclose(thptfp);
   system("hostname > name_trash");
   trs_fp = efopen("name_trash","r");
   fscanf(trs_fp,"%s",hostname);
   fclose(trs_fp);
   system("rm name_trash");
#ifdef GREATSPN
   sprintf(comm,"/bin/rm %s%strash_sim",file,hostname);
#else
   sprintf(comm,"rm nets/%strash_sim",hostname);
#endif
   system(comm);
#ifdef GREATSPN
   sprintf(comm,"/bin/rm %s%strash_time",file,hostname);
#else
   sprintf(comm,"rm nets/%strash_time",hostname);
#endif
   system(comm);
#ifdef GREATSPN
   sprintf(comm,"/bin/rm %s%strash_thp",file,hostname);
#else
   sprintf(comm,"rm nets/%strash_thp",hostname);
#endif
   system(comm);
...
/* RETURN VALUE : */
/**************************************************************/
static void open_intermediate_files(file)
 char file[MAXSTRING];
...
/* RETURN VALUE : */
/**************************************************************/
void forward_sim(file,first_trans_length,trans_length,min_len,max_len,approx,conf)
 char file[MAXSTRING];
 int first_trans_length;
 int trans_length;
 int min_len;
 int max_len;
 int approx;
 ...
 {/* Init forward_sim */
  ...
  while ( again_flag )
   {/* Loop del simulatore */
	...
	again_flag = compute_batch_statistics(ev_type,approx,conf,file);
       }/* Fine del transitorio ed inizio raccolta statistiche */
