...
/* RETURN VALUE : */
/**************************************************************/
void fill_marking_data_structure(char  fname[MAXSTRING])
 {/* Init fill_marking_data_structure */
  ...
  for(i=0;i<npl;i++)
   {/* Per ogni posto */
	...
	num_of_token=1;
        parse_fp = efopen(fname,"w");
        fprintf(parse_fp,"%s",tmp);
        fclose(parse_fp);
        jjin = efopen(fname,"r");
        parser();
#ifdef GSPN
...
#ifdef GREATSPN
 sprintf(comm,"cat %s.net2 %s.net > %s.ntp",argv[1],argv[1],argv[1]);
#else
 sprintf(comm,"cat nets/%s.net2 nets/%s.net > nets/%s.ntp",argv[1],argv[1],argv[1]);
#endif
 system(comm);
...
/* CREAZIONE STRUTTURE DATI PER LE STATISTICHE E LETTURA FILE .stat */
 ...
 compose_name(net,argv[1],"parse");
 fill_marking_data_structure(net);	     /* OPERAZIONI */
